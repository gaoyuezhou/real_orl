...
if __name__ == "__main__":
    ...
    for demo in sorted(name_of_demos):
        ...
        try:
            ...
            csv_data = pd.read_csv(csv_log, names=['timestamp','robostate','robocmd', 'exec_command', 'goal', 'cam'])
        ...
        try: 
            ...
            commands = np.array([np.fromstring(_data, dtype=np.float64, sep=' ') for _data in valid_data['exec_command']]) ### saving exec_cmd for some datasets!
            ...
            if not args.ignore_tracking:
                ...
                for i in range(len(info['jointstates'])):
                    ...
                    if mid_marker_pos[1] < -0.5:
                        import pdb; pdb.set_trace()
                    alpha = 0.25
                    if len(mid_marker_positions) > 0:
                        mid_marker_pos = mid_marker_pos * alpha + mid_marker_positions[-1] * (1 - alpha)
                    mid_marker_positions.append(mid_marker_pos)
                    cur_obs = np.concatenate([info['jointstates'][i], mid_marker_pos])
